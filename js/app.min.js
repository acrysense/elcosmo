...
document.addEventListener('DOMContentLoaded', function () {
  ...
  // service prices accordeon
  var servicePricesAccordeonTrigger = document.querySelectorAll('.service-prices__trigger');
  if (servicePricesAccordeonTrigger) {
    servicePricesAccordeonTrigger.forEach(function (item) {
      item.addEventListener('click', function (event) {
        event.preventDefault();
  ...
  // doctors card icon
  var doctorsCardImg = document.querySelectorAll('.doctors-card__img');
  if (doctorsCardImg) {
    doctorsCardImg.forEach(function (imgContainer) {
      var plusIcon = imgContainer.querySelector('.doctors-card__plus');
      if (plusIcon) {
        var updatePosition = function updatePosition(e, icon, container) {
          var rect = container.getBoundingClientRect();
          var x = e.clientX - rect.left;
          var y = e.clientY - rect.top;
          icon.style.transform = "translate(".concat(x - 20, "px, ").concat(y - 20, "px)");
        };
        imgContainer.addEventListener('mouseenter', function (e) {
          updatePosition(e, plusIcon, imgContainer);
  ...
  var popularServicesItems = document.querySelectorAll('.popular-services__item');
  var popularServicesBgImage = document.querySelector('.popular-services__bg img');
  if (popularServicesItems && popularServicesBgImage) {
    if (popularServicesItems.length > 0) {
      popularServicesBgImage.src = popularServicesItems[0].getAttribute('data-img');
      popularServicesItems[0].classList.add('is--active');
    }
    popularServicesItems.forEach(function (item) {
      item.addEventListener('mouseenter', function () {
        popularServicesItems.forEach(function (i) {
          return i.classList.remove('is--active');
        });
        popularServicesBgImage.src = this.getAttribute('data-img');
      });
    });
    popularServicesWrapper.addEventListener('mouseleave', function () {
      if (popularServicesItems.length > 0) {
        popularServicesItems.forEach(function (i) {
          return i.classList.remove('is--active');
        });
  ...
  if (mainSlider) {
    var myMainSlider = new Swiper(mainSlider, {
      ...
      pagination: {
        el: mainSlider.closest('.main__slider').querySelector('.swiper-pagination'),
        clickable: true
      }
    });
  }
  ...
  // service slider
  var serviceSlider = document.querySelector('.service__slider .swiper');
  if (serviceSlider) {
    var myServiceSlider = new Swiper(serviceSlider, {
      slidesPerView: 1,
      spaceBetween: 20,
      loop: true,
      pagination: {
        el: serviceSlider.closest('.service__slider').querySelector('.swiper-pagination'),
        clickable: true
      }
    });
  }
